...
// Set up the ISO 8583 message data and custom formats
let data = {
    0: "0100",
    2: "4761739001010119",
    3: "000000",
...
const tcpSocket = net.createServer((socket) => {
  socket.on('data', (data) => {
    // Parse the incoming binary buffer to a JSON object

    let incoming =  new iso8583().getIsoJSON(data,{ lenHeaderEncoding: 'hex', bitmapEncoding: 'hex', secondaryBitmap: true })
    ...
    if (incoming[0] === '0100') {
      ...
      socket.write(Buffer.from('1010', 'hex'));
      console.log('1010');
    } else if(incoming[0] === '0800') {
        // This is a ping request
        // Extract the relevant data from the message
        socket.write(Buffer.from('0810', 'hex'));
        console.log('0810');
    }
    else {
      // This is an unsupported message type
